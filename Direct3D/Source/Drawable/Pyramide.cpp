	...
	phi(adist(rng))
{
	...
	if (!IsStaticInitialized())
	{
		...
		auto pvs = std::make_unique<VertexShader>(gfx, L"BlendVertexShader.cso");
		auto pvsbc = pvs->GetBytecode();
		AddStaticBind(std::move(pvs));

		AddStaticBind(std::make_unique<PixelShader>(gfx, L"BlendPixelShader.cso"));
...
DirectX::XMMATRIX Pyramid::GetTransfromXM() const noexcept
{
	namespace dx = DirectX;
